...
async function t(){
    try{
        ...
        let downloaded = 0
        const video = youtubedl(ans, ['--format=18', '-f', "best[ext=mp3]/best"], { start: downloaded, cwd: __dirname })
        video.on('info', function(info) {
          ...
          output = "./audios/" + info._filename.split(" ").join("_").replace(/[&\/\\#!,+()$~%.'\s":*?<>{}]/g,'_').substr(0, 30) + ".mp3";
        ...
        video.on('end', function() {
          ...
          require('child_process').exec('start "" "audios"');
